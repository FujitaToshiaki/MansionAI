...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Revision groups endpoint
  app.get("/api/revision-groups", async (req, res) => {
    try {
      const query = `
        SELECT * FROM revision_groups 
        ORDER BY created_at DESC
      `;
      const result = await db.execute(query);
      res.json(result.rows);
    } catch (error) {
      console.error('Error fetching revision groups:', error);
      res.status(500).json({ error: "Failed to fetch revision groups" });
    }
  });

  // Single revision group endpoint
  app.get("/api/revision-groups/:id", async (req, res) => {
    try {
      const query = `
        SELECT rg.*, 
      ...
      console.error('Error fetching revision group:', error);
      res.status(500).json({ error: "Failed to fetch revision group" });
    }
  });

  // Regulation revisions by group endpoint  
  app.get("/api/revision-groups/:id/revisions", async (req, res) => {
    try {
      const query = `
  ...
  // Individual regulation revision endpoint
  ...
  // Regulation revisions endpoint (legacy for backward compatibility)
