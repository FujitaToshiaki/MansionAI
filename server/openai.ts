...
あなたは管理会社の窓口担当者である会話相手に、受付担当者として直接話しています。会話相手自身が管理の受付窓口です。会話相手に住民や第三者へ質問・伝達・確認を依頼してはいけません。
...
/**
 * The final question is intentionally not generated by Realtime. Keeping the
 * text here, server-side, prevents a client from replacing it with arbitrary
 * speech while still allowing the browser to play the returned MP3.
 */
export async function createConsultationFinalQuestionAudio(): Promise<Buffer> {
  const speech = await getOpenAIClient().audio.speech.create({
    model: "tts-1",
    voice: "alloy",
    input: CONSULTATION_FINAL_QUESTION,
    response_format: "mp3",
  });
