import { Search, Bell, ChevronDown, User, Building, ChevronRight } from "lucide-react";
...
/** Pattern-based rules (longest prefix wins) */
const PREFIX_RULES: Array<{ prefix: string; info: NavInfo }> = [
  { prefix: "/condominiums/", info: { parent: "物件管理", title: "物件詳細" } },
  { prefix: "/standard-regulations/", info: { parent: "管理規約管理", title: "標準管理規約" } },
  { prefix: "/revision-years/", info: { parent: "管理規約管理", title: "年度別改訂管理" } },
  { prefix: "/ai-agent-history/", info: { parent: null, title: "AI実行履歴" } },
];

function CONDOMINIUM_SUFFIX_MAP(suffix: string): NavInfo {
  const map: Record<string, NavInfo> = {
  ...
  return map[suffix] ?? { parent: "物件管理", title: "物件詳細" };
}

function getNavInfo(location: string): NavInfo {
  if (EXACT_MAP[location]) return EXACT_MAP[location];

  // /condominiums/:id/suffix
  const condoSuffixMatch = location.match(/^\/condominiums\/[^/]+\/(.+)$/);
  if (condoSuffixMatch) return CONDOMINIUM_SUFFIX_MAP(condoSuffixMatch[1].split("/")[0]);

  // prefix rules
  for (const rule of PREFIX_RULES) {
    if (location.startsWith(rule.prefix)) return rule.info;
  }
...
/* ------------------------------------------------------------------ */
/*  Extract condominiumId from current URL                              */
/* ------------------------------------------------------------------ */
function extractCondominiumId(location: string, search: string): string | null {
  ...
  // Path param: /condominiums/:id or /condominiums/:id/...
...
export default function Header() {
  ...
  return (
    <header className="bg-white border-b border-gray-200 h-16 flex items-center justify-between px-6">
      {/* Left: breadcrumb title + condominium chip */}
      ...
      <div className="flex items-center space-x-3 flex-shrink-0">
        ...
        <div className="relative hidden md:block">
          ...
          <Input
            type="text"
            placeholder="検索..."
            className="bg-gray-100 border-0 rounded-lg px-4 py-2 pl-9 text-sm w-52"
          />
        </div>
